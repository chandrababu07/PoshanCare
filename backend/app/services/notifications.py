...
async def generate_user_notifications(db: AsyncSession, user_id: int) -> List[HealthNotification]:
    ...
    Deduplicates based on 24-hour window per rule source.
    """
    created_notifications: List[HealthNotification] = []

    # 0. Fetch user profile for persona safety rules
    ...
    if not today_meals:
        ...
        # Check consecutive logging days
    ...
    # 6. WEIGHT TREND RULE (Adults only)
    if profile_type not in ["child", "teen"]:
